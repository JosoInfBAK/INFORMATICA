...
void main (void)
{
    ...
    //Creación de los cincos hilos para led orange, green, blue1, blue2, blue3
    //La prioridad indica quien sale antes
    //Creamos el task y se añade a la cola de task
    //El task mayor prioridad saldrá antes
    OSTaskCreate(        
                (OS_TCB     *)&task_led1_TCB,
    ...
    //Semaphore create
    /*
         OSSemCreate ((OS_SEM *) &MySemaphore,
            (CPU_CHAR *) "Semaforo de sincronizacion".
            (OS_SEM_CTR) 0,
            (OS_ERR *) &os_err);
     */
   ...
   OSSemCreate ((OS_SEM  *) &MySemaphore,
             (CPU_CHAR  *) "Semaforo de sincronizacion",
             (OS_SEM_CTR ) 0, //valor inicial (1 = Disponible)
             (OS_ERR    *) &os_err);
...
//Semaphore post y pend --> debe estar dentro del task 1 donde mutex.

/*
 OSSemPend(&MySemaphore,
        0,
...
void TASK_LED1 (void)
{
    ...
    { 
       ...
       OSSemPend   (&MySemaphore,          // Esperamos por el semaforo para resetear disponible
             0,                     // No timeout
             OS_OPT_PEND_BLOCKING,  //Esperar hasta que el semaforo sea liberado o el timeout expire
             &ts,                   //timestamp (para saber cuando se ha liberado el recurso (OS_TS_GET()))
             &os_err);
        ...
        OSSemPost   (&MySemaphore, //Liberar recurso compartido (semaforo = 1)
             OS_OPT_POST_1,
             &os_err);
...
void TASK_LED3 (void)
{
    ...
    while(1)
    {
        //sprintf(txdata,"Practica 4 prueba de tarea 33333333 dice holaa\r\n");
        //EnviarString(txdata, Uart3);
...
void TASK_LED4 (void)
{
    ...
    while(1)
    {
       // sprintf(txdata,"Practica 4 prueba de tarea 44444444 dice holaa\r\n");
       // EnviarString(txdata, Uart3);
...
void TASK_LED5 (void)
{
    ...
    while(1)
    {
       // sprintf(txdata,"Practica 4 prueba de tarea 55555555 dice holaa\r\n");
        //EnviarString(txdata, Uart3);
...
void TASK_LED6 (void)
{
    ...
    while(1)
    {
       // sprintf(txdata,"Practica 4 prueba de tarea 66666666 dice holaa\r\n");
        //EnviarString(txdata, Uart3);
