...
#define baud_4800  1041   // brgh= 1 / Fcpu=20mhz
...
void uart_config(unsigned int baud) {
    ...
    TRISBbits.TRISB3 = 1;
    TRISBbits.TRISB2 = 0;
    RPOR1bits.RP2R = 3; //U1TX conetada al pin RP2 (RB2) para transmitir
    RPINR18bits.U1RXR = 3; //U1RX conectada al pin RP3 (RB3) para recibir, esto va en el slave
...
int main(void) {
    ...
    while (1) {
        if (U1STAbits.URXDA) {
            var_recv = U1RXREG; //este dato va a salir por RB2
            for (int i = 0; i < 10; i++) {
                if (txbuffer[i] == NULL) {
                    if (var_recv == 8) {
                        txbuffer[i - 1] = NULL;
